...
- 引用**錨定穩定位置**：優先指函式／類別定義的開頭行，而不是函式體中間的某一行——
  上游插一行，體內行號全漂，定義行相對穩定。引用範圍寧可 `f.py:100-130`（整個函式）
  也不要 `f.py:117`（某個 if）。
...
- **內部代號必須有錨點**：版本迭代號（v48）、規格編號（§3.1）、內部縮寫（T-SYM）這類
  只有團隊老手懂的代號，要嘛在 overview 詞彙表給一行解釋，要嘛就地展開，要嘛刪掉——
  指向 docs/ 外部私有文件的引用對新讀者是死連結。
...
8. 內部代號（版本號／規格編號／縮寫）都有詞彙表錨點或就地解釋？
