...
3. **README 連結健檢**（零成本，早退前做）：掃 repo README 裡指向 docs/ 的相對
   連結與檔名引用，目標不存在的列入報告末尾（獨立一節「README 斷鏈」）——README
   的文件索引指向已被移除的檔案，是讀者第一個撞到的 drift。
4. **早退**：若所有 docs 的 status 皆為 `clean`、`unmatchedFiles` 為空、且無
   README 斷鏈，回報「無 drift，文件與程式碼對齊」並結束。
5. **逐文件驗證**：對每份 status 為 `affected` 的文件，依 `type` 選擇方法。
   `unverified` 的文件視同全量：對整份文件做同樣驗證，並在報告註明原因
   （缺少或失效的 last_verified）。若 repo 有程式碼索引工具（如 codegraph）優先用它
   查 symbol 與呼叫路徑；否則搜尋原始碼。行為規則段落逐條驗證（見步驟 6）；
...
6. **行為規則與敘事段落驗證**：驗證圖的同時，逐條檢查該文件「行為規則」段落裡的行為
   宣告（「當 X 條件成立時系統會 Y」）是否仍與程式碼一致；「目的與情境」「設計決策」
   等敘事段落依步驟 5 的意圖層原則，僅在觸發結構性變動判準時一併檢查。
7. **產出報告**（格式見下）。報告本身輸出給使用者，不寫入檔案。
...
- **README 斷鏈**（有斷鏈時）：步驟 3 發現的失效連結，逐條列出並附建議修法。
