...
//
// 用法：node llm-check.js --range <git range> [--out <path>] [--manifest <path>]
...
function parseArgs(argv) {
  const opts = { manifest: 'docs/.docalign.yml', out: null, range: null };
  ...
  if (!opts.range) throw new Error('llm-check requires --range <git range> (e.g. origin/main...HEAD)');
  return opts;
}
...
async function main() {
  ...
  if (broken.length) throw new Error(`llm-check: cannot diff range ${opts.range}: ${broken[0].reason}`);

  const affected = scope.docs.filter((d) => d.status === 'affected');
  const report = [];

  if (affected.length === 0) {
    report.push('無 drift，文件與程式碼對齊（本次變動未觸及任何文件的 watch 範圍，未呼叫 LLM）。');
  } else {
    ...
    for (const d of affected) {
      ...
      const { prompt, truncated } = buildCheckPrompt({
        ...
        diff: gitDiff(opts.range, d.matchedFiles),
